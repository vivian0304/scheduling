void insert_max_heap(HeapType *h, process_in item) //최대 히프: 부모 노드의 키값이 자식 노드 키값보다 크거나 같음.
                                                    //현재 요소의 개소가 heap_size인 히프 h에 item 삽입한다.
{
    int i;
    i = ++(h->heap_size);
    
    while ((i != 1) && (item.priority > h->heap[i / 2].priority)) //우선순위 비교해서 재정렬, 트리를 다시 올라가면서 부모 노드와 비교
    {
...
process_in delete_max_heap(HeapType *h) 
{
    ...
    while (child <= h->heap_size)
    {
        ...
        if ((child < h->heap_size) &&
            ...
            child++;
        if (temp.priority >= h->heap[child].priority) //우선순위 판단
            break;
        //한단계 아래로 이동
...
HeapType *heap_sort(process_in a[], int n) //히프 정렬 실행
{
    int i;
    HeapType *h;
    ...
    for (i = 0; i < n; i++)
    {
        insert_max_heap(h, a[i]); //입력받은 프로세스 수만큼 삽입
    }
    for (i = (n - 1); i >= 0; i--)
    {
        a[i] = delete_max_heap(h); //삭제
...
double priority(process_in a[], int n){
    printf("=================================================PRIORITY=================================================\n");

    heap_sort(a,n);
    double At,final = 0;
    
    printf("| process_id | priority | computing_time  | turn_around_time | arrival_time | kill_time | 정규화 반환 시간 |\n");
    printf("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n");
